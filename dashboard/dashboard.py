...
# ─────────────────────────────────────────
# SECTION 2 — KATEGORI PRODUK (PLOTLY)
# ─────────────────────────────────────────
...
fig1 = make_subplots(
    rows=1, cols=2,
    subplot_titles=(
        f"Top {top_n} Kategori — Volume Penjualan",
        f"Top {top_n} Kategori — Revenue Terbesar"
    )
)
...
fig1.add_trace(
    go.Bar(
        x=top_volume["order_count"].iloc[::-1],          # ← balik urutan
        y=top_volume["product_category_name_english"].iloc[::-1],  # ← balik urutan
        orientation="h",
        marker_color=colors_vol[::-1],                   # ← balik warna juga
...
fig1.add_trace(
    go.Bar(
        x=top_revenue["revenue"].iloc[::-1],             # ← balik urutan
        y=top_revenue["product_category_name_english"].iloc[::-1],  # ← balik urutan
        orientation="h",
        marker_color=colors_rev[::-1],                   # ← balik warna juga
...
fig1.update_layout(
    title_text="Performa Kategori Produk: Volume vs Revenue",
    title_font_size=18,
    showlegend=False,
    height=500
)
...
with st.expander("📝 Insight Kategori Produk"):
    st.markdown("""
    - **bed_bath_table** memimpin volume penjualan — produk rumah tangga adalah penggerak utama transaksi harian.
    - **health_beauty** & **watches_gifts** unggul di sisi revenue karena harga per unit yang lebih tinggi.
    - **Implikasi bisnis:** Strategi stok & promosi harus dibedakan antara *volume-driven* dan *revenue-driven* kategori.
    """)

st.markdown("---")

# ─────────────────────────────────────────
# SECTION 3 — POLA WAKTU (PLOTLY)
# ─────────────────────────────────────────
...
fig2 = make_subplots(
    rows=1, cols=2,
    subplot_titles=("Distribusi Pesanan per Jam", "Distribusi Pesanan per Hari")
)

colors_hour = ["#0077b6" if h == peak_hour else "#adb5bd" for h in hourly_orders["order_hour"]]
fig2.add_trace(
    go.Bar(
...
colors_day = ["#e67e22" if d == peak_day else "#adb5bd" for d in daily_orders["order_day"]]
fig2.add_trace(
    go.Bar(
...
fig2.update_layout(
    title_text="Pola Waktu Pemesanan Pelanggan",
    title_font_size=18,
    showlegend=False,
    height=450
)
...
fig3 = px.imshow(
    ...
    title="Heatmap Kepadatan Waktu Pemesanan (Hari × Jam)"
)
...
with st.expander("📝 Insight Pola Waktu"):
    st.markdown("""
    - Puncak pemesanan terjadi antara **pukul 10:00–17:00**, dengan jam tertinggi di sekitar pukul 14:00–16:00.
    - **Senin & Selasa** adalah hari paling aktif; **Sabtu & Minggu** paling sepi.
    - **Implikasi bisnis:** Jadwalkan flash sale, push notification, dan kampanye iklan di rentang jam tersebut.
    """)

st.markdown("---")

# ─────────────────────────────────────────
# SECTION 4 — RFM (PLOTLY)
# ─────────────────────────────────────────
...
col_r2.metric("🏆 Champions", f"{len(rfm[rfm['segment'] == 'Champions']):,}")
...
fig4 = make_subplots(
    rows=1, cols=2,
    subplot_titles=("Jumlah Pelanggan per Segmen", "Rata-rata Monetary per Segmen (BRL)")
)
...
fig4.add_trace(
    go.Bar(
        ...
        orientation="h",
        marker_color=[segment_colors.get(s, "#adb5bd") for s in segment_counts["segment"]],
...
fig4.add_trace(
    go.Bar(
        ...
        orientation="h",
        marker_color=[segment_colors.get(s, "#adb5bd") for s in segment_monetary["segment"]],
...
fig4.update_layout(
    title_text="Analisis Segmentasi Pelanggan RFM",
    title_font_size=18,
    showlegend=False,
    height=500
)
...
fig5 = px.scatter(
    ...
    title="Sebaran Pelanggan: Recency vs Monetary",
    opacity=0.6,
    height=500
)
