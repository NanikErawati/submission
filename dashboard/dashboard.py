...
### 📋 Isi Dashboard
- 📌 Overview Performa
- 📦 Volume & Revenue Tertinggi
- ⏰ Pola Waktu Pemesanan
- 👥 Segmentasi Pelanggan RFM

**Periode Analisis: 2017–2018**
""")
st.sidebar.markdown("---")
...
# ─────────────────────────────────────────
st.markdown("""
    <h1 style='text-align: center;'>🛒 Brazilian E-Commerce Dashboard</h1>
    <p style='text-align: center; font-size: 18px;'>
        Analisis performa penjualan, pola waktu pemesanan, dan segmentasi pelanggan 
        berbasis data <b>Olist E-Commerce</b> periode <b>2017–2018</b>.
    </p>
...
avg_order_val = df_clean.groupby("order_id")["price"].sum().mean()
...
df_waktu = df_clean.copy()
df_waktu["order_hour"] = df_waktu["order_purchase_timestamp"].dt.hour
df_waktu["order_day"]  = df_waktu["order_purchase_timestamp"].dt.day_name()

hourly_orders = df_waktu.groupby("order_hour")["order_id"].nunique().reset_index()
hourly_orders.rename(columns={"order_id": "order_count"}, inplace=True)

days_order = ["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"]
daily_orders = df_waktu.groupby("order_day")["order_id"].nunique().reindex(days_order).reset_index()
